...
class InsideEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.ext_linear = nn.Linear(config.hidden_size, 1)
    ...
    def forward(self, src, span_embeds=None):
        ...
        :return: mat_scores, output_repr
        """
        dim = src.shape[-1]
        org_shape = src.shape  # (batch_size, comb_size, 2, dim)
        if span_embeds is not None:
            # [ L | R | ext ] -> encoder_layers
            span_embeds = span_embeds.unsqueeze(1).unsqueeze(2).expand(-1, org_shape[1], 1, -1)
            output = torch.cat((src, span_embeds), dim=2)
            output = output.view(-1, 3, dim)
        else: # deprecated
            # [ L | R ] -> encoder_layers
        ...
        ## NOTE: new score
        mat_scores = self.ext_linear(output[:, 2, :]) # (batch x comb, dim) -> (batch x comb, 1)
        mat_scores = mat_scores.view(*org_shape[:-2]) # (batch_size, comb_size)
    
        return mat_scores, self.norm(torch.sum(output[:,:2,:], dim=1).view(*org_shape[:-2], dim))
