...
class GPT2Dataset(data.Dataset):
    ...
    def getidx(self, data_idx):
        ...
        # TODO: make another copy of token_ids, wrap super long segments with special tokens
...
class EvalSentDataset(data.Dataset):
...
class CharGPT2Dataset(data.Dataset):
    ...
    def __init__(self, ds,
        ...
        if hasattr(self.ds, 'is_lazy') and self.ds.is_lazy:
            self.is_lazy = True
        self.init_weighting()
        # handle empty string after stripping in collator
    ...
    def __getitem__(self, idx):
        ...
        def convert_ids_to_str(word_ids):
            # return ' '.join(['a']*len(word_ids)) # abla
            return ' '.join([self.ds.idx2vocab[idx] for idx in word_ids])
        ...
        data_idx = self.get_weighted_samples(rng)
        #        data_idx = rng.choice(self.ds_len, p=self.weighting)
        word_ids = self.getidx(data_idx)
        input_str = convert_ids_to_str(word_ids)
        ...
        if tokens_to_strip > 0:
            strip_left_tokens = rng.randint(tokens_to_strip + 1)
            input_str = input_str[strip_left_tokens:]

            strip_right_rokens = len(input_str) - self.max_seq_len
            if strip_right_rokens > 0:
                input_str = input_str[:-strip_right_rokens]

        # Sample multiple documents
        if self.sample_across_doc:
            while (len(input_str) < self.max_seq_len):
                if self.random_across_doc_sampling:
                    data_idx = self.get_weighted_samples(rng)
                else:
                    data_idx = (data_idx + 1) % self.ds_len
                new_word_ids = self.getidx(data_idx)
                new_input_str = convert_ids_to_str(new_word_ids)
                input_str = input_str + new_input_str

            input_str = input_str[:self.max_seq_len]

        # Handle Empty string in collator
        # while len(input_str.split()) <= 2: 
        #     input_str = self.__getitem__(self.num_samples+self.exception_cnt)
        #     self.exception_cnt += 1 # empty string in collator
