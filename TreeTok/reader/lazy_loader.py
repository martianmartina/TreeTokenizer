...
class LazyLoader(object):
    ...
    def __init__(self, path, data_type='data', is_array=False, sent_level=False, array_data_type=np.int32, skip_size=0):
        ...
        # if operates on chars
        idx2vocab_path = os.path.join(lazypath, 'idx2vocab.pkl')
        vocab2freq_path = os.path.join(lazypath, 'vocab2freq.pkl')
        vocab2idx_path = os.path.join(lazypath, 'vocab2idx.pkl')
        if os.path.exists(idx2vocab_path):
            with open(idx2vocab_path, 'rb') as file:
                self.idx2vocab = pkl.load(file)
                print("word dict size", len(self.idx2vocab))
        
        if os.path.exists(vocab2freq_path):
            with open(vocab2freq_path, 'rb') as file:
                self.vocab2freq = pkl.load(file)
        
        if os.path.exists(vocab2idx_path):
            with open(vocab2idx_path, 'rb') as file:
                self.vocab2idx = pkl.load(file)
        ...
        if not sent_level:
            ...
            self.lens = list(filter(lambda x: x > 0, self.lens))
