...
class CPPChartTableManager:
    ...
    def best_trees(self, split_orders, group_ids, max_input_len, atom_spans=None):
        ...
        span_masks, splits = self.cpp_tbl_mgr.best_trees(split_orders, atom_spans, group_ids, max_input_len)
        return span_masks, splits
    
    def prepare_bilm(self, total_len, bos_id, eos_id):
        return self.cpp_tbl_mgr.prepare_bilm(total_len, bos_id, eos_id)
    
    def get_cached_bij(self):
        return self.cpp_tbl_mgr.get_cached_bij()
    
    def prepare_span_prediction(self, a_ij, non_overlapping=True):
        # weights torch.tensor in torch.float32
        # cache_ids: torch.tensor in torch.long
        # span ids: torch.tensor in torch.long
        split_probs = [probs.data.numpy() for probs in a_ij]
        weights, cache_ids, span_ids = self.cpp_tbl_mgr.prepare_span_prediction(split_probs, non_overlapping)
        return weights, cache_ids, span_ids