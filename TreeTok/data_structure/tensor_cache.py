...
class TensorCache:
    def __init__(self, max_window,
        ...
        for i, cache_type in enumerate(cache_types):
            ...
            elif cache_type == CacheType.INTEGER:
                self.caches[i] = torch.full((self.total_block_size * 2, dims[i]), 0, dtype=torch.long, device=device)
    ...
    def scatter(self, indices, cache_ids, values):
        ...
        for cache_id, value in zip(cache_ids, values):
            ...
            if torch.is_autocast_enabled() and value.dtype != tensor_block.dtype:
