...
__global__ void init_ranges(CellRange * ranges, scalar_t * seq_lens) {
    ...
    ranges[batch_i].batch_id_offset = 0;
}
...
__global__ void update_batch_id_offsets(CellRange * ranges, uint current_beam_size) {
    int batch_id = blockIdx.x;
    int nodes_len = ranges[batch_id].end - ranges[batch_id].start + 1;
    ranges[batch_id].batch_id_offset += nodes_len * current_beam_size;
}
...
template <typename scalar_t>
__global__ void gather_root_ids(scalar_t * root_ids, 
                                TableCell ** all_cells, 
                                uint **right_most) {
    int batch_i = threadIdx.x;
    uint j = right_most[batch_i][0];
    root_ids[batch_i] = all_cells[batch_i][j].beams[0].cache_id;
}
...
__global__ void gather2d(torch::PackedTensorAccessor32<scalar_t1, 4,torch::RestrictPtrTraits> src, 
                         torch::PackedTensorAccessor32<scalar_t1, 4,torch::RestrictPtrTraits> tgt,
                         torch::PackedTensorAccessor32<scalar_t2, 3,torch::RestrictPtrTraits> index, 
                         torch::PackedTensorAccessor32<scalar_t2, 1,torch::RestrictPtrTraits> id2batch,
                         int topk, int head_num, int head_dim) {
    int block_id = blockIdx.x;
    int thread_idx = threadIdx.x;
    int token_i = block_id / topk;
    int seq_idx = block_id % topk;
    int head_idx = -1;
    int dim_idx = -1;
    while (thread_idx < head_dim * head_num) {
        head_idx = thread_idx / head_dim;
        dim_idx = thread_idx % head_dim;
        tgt[token_i][seq_idx][head_idx][dim_idx] = src[id2batch[token_i]][index[token_i][seq_idx][head_idx]][head_idx][dim_idx];
        thread_idx += blockDim.x;
    }
}
...
template <typename scalar_t1, typename scalar_t2>
__global__ void recover_sampled_trees(scalar_t1 * span_masks, scalar_t1 * targets, scalar_t1 * split_points,
                                      TableCell ** all_cells, scalar_t2 * seq_lens, TableCell ** caches,
                                      size_t num_samples, size_t max_len, size_t split_points_len) {
    int batch_i = blockIdx.x;
    int sample_id = threadIdx.x;
    scalar_t2 seq_len = seq_lens[batch_i];
    ...
    scalar_t1 * span_mask_offset = span_masks + max_len * max_len * num_samples * batch_i + max_len * max_len * sample_id;
    scalar_t1 * target_offset = targets + max_len * num_samples * batch_i + max_len * sample_id;
    int front = 0;
...
template <typename scalar_t1, typename scalar_t2>
__global__ void recover_split_cache_ids(torch::PackedTensorAccessor32<scalar_t1, 2,torch::RestrictPtrTraits> split_points, 
                                        torch::PackedTensorAccessor32<scalar_t1, 2,torch::RestrictPtrTraits> split_indices, 
                                        torch::PackedTensorAccessor32<scalar_t1, 2,torch::RestrictPtrTraits> split_cache_ids,
                                        scalar_t2 * seq_lens, TableCell ** all_cells, TableCell ** caches, size_t max_len) {
    // Please note: different from recover_sampled_trees, we hope to recover all cache_ids of  terminal nodes and non-terminal nodes.
    // So the total size for each split_cache_ids is 2 * n - 1 instead of n - 1.
    int batch_i = blockIdx.x;
    scalar_t2 seq_len = seq_lens[batch_i];
    TableCell ** cache = caches + batch_i * max_len;
    // split_points: (total_cells, K)
    // span_masks: (batch_size, K, L, L)
    // target: (batch_size, K, L)
    int front = 0;
    ...
    while(front > 0) {
        ...
        front--;
        if (top->i < top->j) {
            // ASSERT(top->cell_idx < split_points_len);
            ASSERT(split_points[top->cell_idx][0] < top->j - top->i);
            k = top->splits[split_points[top->cell_idx][0]];
            ASSERT(k >= top->i);
            ASSERT(k < top->j);
            split_indices[batch_i][step] = k;
            // TODO: not applicable for beam version
            split_cache_ids[batch_i][step] = top->beams[0].cache_id;
            // preorder traversal
            if (k + 1 <= top->j) {
                cache[front] = &all_cells[batch_i][(k + 1) * seq_len + top->j];
                front++;
...
template <typename scalar_t>
__global__ void gather_span_bilm(scalar_t * cache_ids,
                                 scalar_t * spans_batch,
                                 TableCell ** all_cells,
                                 uint **left_most, 
                                 uint **right_most,
                                 scalar_t * seq_lens,
    ...
    int batch_i, cell_i, cell_j, beam_i;
    int seq_len;

    while (thread_i < total_len) {
        cache_id_offset = 2 * thread_i;
        batch_i = spans_batch[thread_i * 3];
        cell_i = spans_batch[thread_i * 3 + 1];
        cell_j = spans_batch[thread_i * 3 + 2];
        beam_i = thread_i % beam_size;
        seq_len = seq_lens[batch_i];
        if (cell_i == 0) {
            cache_ids[cache_id_offset] = bos;
        } else {
            cache_ids[cache_id_offset] = get_cache_id(all_cells[batch_i], 
        ...
        if (cell_j == seq_len - 1) {
            ...
            cache_ids[cache_id_offset + 1] = get_cache_id(all_cells[batch_i],
                cell_j + 1, right_most[batch_i][cell_j + 1], beam_i, seq_len, empty_cache_id);
        }
        thread_i += blockDim.x;
    }
}
...
template<typename scalar_t>
__global__ void update_batch_indices(scalar_t * batch_indices,
                                     scalar_t * seqlen_cum_sum,
                                     uint id_offset,
                                     int max_len,
                                     uint current_beam_size,
                                     CellRange * ranges) {
    int batch_i = blockIdx.x;
    int idx = threadIdx.x;
    CellRange * current_range = &ranges[batch_i];
    int st = current_range->batch_id_offset;
    int ed = st + (current_range->end - current_range->start + 1) * current_beam_size;
    int batch_id_offset = 0;
    if (batch_i > 0) {
        batch_id_offset = id_offset + (int)seqlen_cum_sum[batch_i - 1] * current_beam_size;
    } else {
        batch_id_offset = id_offset;
    }

    while (idx < ed) {
        if (idx >= st) {
            batch_indices[batch_i * max_len + idx] = batch_id_offset + idx - st;
        }
        idx += blockDim.x;
    }
}
...
void cuda_gather_span_bilm(const at::Tensor & cache_ids, 
                           const at::Tensor & spans_batch,
                           TableCell **all_cells,
                           uint ** left_most, 
                           uint ** right_most,
                           uint empty_cache_id,
                           ...
                           uint bos,
                           uint eos) {
    // spans_batch: (N, 3): N groups of (batch_id, i, j)
    uint spans_size = spans_batch.size(0);
    AT_DISPATCH_INTEGRAL_TYPES(cache_ids.scalar_type(), "gather_span_bilm", ([&] {
        gather_span_bilm<<<1, min(MAX_THREAD, spans_size)>>>(cache_ids.data_ptr<scalar_t>(), 
                    spans_batch.data_ptr<scalar_t>(),
                    all_cells, left_most, right_most, seq_lens.data_ptr<scalar_t>(), empty_cache_id, beam_size, bos, eos, spans_size);
    }));
}
...
void cuda_recover_sample_trees(const at::Tensor & span_masks,
    ...
    if (targets.size(2) > 0) {
        ...
        AT_DISPATCH_INTEGRAL_TYPES(seq_lens.scalar_type(), "recover_sample_trees", ([&] {
            typedef scalar_t scalar_t2;
            AT_DISPATCH_INTEGRAL_TYPES(split_points.scalar_type(), "recover_sample_trees", ([&] {
                recover_sampled_trees<<<span_masks.size(0), span_masks.size(1)>>>(span_masks.data_ptr<scalar_t>(), 
                    ...
                    seq_lens.data_ptr<scalar_t2>(), caches, span_masks.size(1), span_masks.size(2),
...
void cuda_recover_split_cache_ids(const at::Tensor & split_points, 
                                  const at::Tensor & split_indices, 
                                  const at::Tensor & split_cache_ids,
                                  TableCell ** all_cells,
                                  at::Tensor seq_lens) {
    // split_points: (total_cell_size, 1) the given split points of a sentences
    // split_indices: (batch_size, max_len - 1)
    TableCell ** caches;
    HANDLE_ERROR(cudaMalloc(&caches, split_indices.size(0) * split_indices.size(1) * sizeof(TableCell*)));

    AT_DISPATCH_INTEGRAL_TYPES(seq_lens.scalar_type(), "recover_split_cache_ids", ([&] {
        typedef scalar_t scalar_t2;
        AT_DISPATCH_INTEGRAL_TYPES(split_indices.scalar_type(), "recover_split_cache_ids", ([&] {
            recover_split_cache_ids<<<split_indices.size(0), 1>>>(
                split_points.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
                split_indices.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
                split_cache_ids.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(), 
                seq_lens.data_ptr<scalar_t2>(), all_cells, caches, split_indices.size(1));
        }));
    }));
    HANDLE_ERROR(cudaFree(caches));
}

void cuda_gather_root_ids(const at::Tensor & root_ids, 
                          TableCell ** all_cells, 
                          uint **right_most) {
    const uint batch_size = root_ids.size(0);
    AT_DISPATCH_INTEGRAL_TYPES(root_ids.scalar_type(), "gather_root_ids", ([&] {
        gather_root_ids<<<1, batch_size>>>(root_ids.data_ptr<scalar_t>(), all_cells, right_most);
    }));
}

void cuda_gather2d(const at::Tensor & src, 
                   const at::Tensor & index,
                   const at::Tensor & tgt,
                   const at::Tensor & id2batch) {
    //ASSUMPTION: src shape: (batch_size, max_seq_len, head_num, head_dim)
    //            index shape: (total_num, topk, head_num)
    //            tgt shape: (total_num, topk, head_num, head_dim)
    //            id2batch shape: (total_num)
    const uint total_num = index.size(0);
    const uint topk = index.size(1);
    const uint head_num = index.size(2);
    const uint head_dim = src.size(3);
    AT_DISPATCH_INTEGRAL_TYPES(index.scalar_type(), "gather2d_int", ([&] {
        typedef scalar_t scalar_t1;
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(src.scalar_type(), "gather2d_float", ([&] {
            gather2d<<<total_num * topk, min(MAX_THREAD, head_num * head_dim)>>>(
                src.packed_accessor32<scalar_t, 4,torch::RestrictPtrTraits>(), 
                tgt.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
                index.packed_accessor32<scalar_t1, 3, torch::RestrictPtrTraits>(), 
                id2batch.packed_accessor32<scalar_t1, 1, torch::RestrictPtrTraits>(), 
                topk, head_num, head_dim);
        }));
    }));
}