...
vector<uint> TablesManager::step(const at::Tensor & cache_ids, const at::Tensor & log_p_ids, 
                                 const at::Tensor & span_lens, const at::Tensor & bigram_score_cache, 
...
void TablesManager::step_over(const at::Tensor & log_p_batch, const at::Tensor & candidates_log_p)
{
    ...
    if (this->m_iCurrentStep >= this->m_iWindowSize)
    {
        ...
        if (!this->m_bHasTrajectories && !this->m_bBigramOnly) {
...
void TablesManager::step_over_trivial()
{
    // update log_p of tree nodes
    int current_max_update_len = this->m_iMaxSeqlen;
    if (this->m_iCurrentStep > this->m_iWindowSize)
...
void TablesManager::prepare_span_bilm(const at::Tensor & cache_ids, const at::Tensor & spans_batch, uint bos, uint eos)
{
    cuda_gather_span_bilm(cache_ids, spans_batch, this->m_pCells, this->m_pLeftmost, this->m_pRightmost,
                     this->m_iEmptyCacheId, this->m_iBeamSize, this->m_tSeqlens, bos, eos);
}
...
void TablesManager::recover_split_cache_ids(const at::Tensor & split_points,
                                            const at::Tensor & split_indices,
                                            const at::Tensor & split_cache_ids) {
    cuda_recover_split_cache_ids(split_points, split_indices, split_cache_ids, this->m_pCells, this->m_tSeqlens);
}

void TablesManager::set_bigram_only()
{
    this->m_bBigramOnly = true;
}

void TablesManager::gather_root_ids(const at::Tensor & root_ids) {
    cuda_gather_root_ids(root_ids, this->m_pCells, this->m_pRightmost);
}

void gather2d(const at::Tensor & src, 
              const at::Tensor & index,
              const at::Tensor & tgt,
              const at::Tensor & id2batch) {
    cuda_gather2d(src, index, tgt, id2batch);
}