...
best_split{-1}, a_ij_split{-1}, cache_id{-1}, batch_id(batch_id), m_pReadyChild(0), m_pMgr(mgr), m_bIsRoot(is_root), ext_vocab_id(0), span_weight(0) {
    this->m_pNode = NULL;
    if (j > i) {
...
m_iCacheOffset{cache_id_offset}, m_iCurrentStep{1}, m_iDetachCacheOffset(detach_id_offset), m_iCellNum{0} {
    ...
    // variables assigned in this->best_trees
    // this->m_pCache_ids_long = NULL;
    // this->m_pSplit_ids_long = NULL;
        ...
        if (span_ids.size() > 0) {
            ...
            for (int span_idx = 0; span_idx < total_size; ++span_idx) {
                ...
                if (!table->isEmpty(span_ids_ptr[span_idx * 3], span_ids_ptr[span_idx * 3 + 1])) {
                    ...
                    for (int span_i = span_ids_ptr[span_idx * 3]; span_i < span_ids_ptr[span_idx * 3 + 1]; ++span_i){
                        for (int span_j = span_i + 1; span_j <= span_ids_ptr[span_idx * 3 + 1]; ++span_j){
                            if (table->get(span_i, span_j)->ext_vocab_id == 0){
                                assert (span_j > span_i >= 1);
                                table->get(span_i, span_j)->ext_vocab_id = -100;
                            }
                        }
                    }
                }
...
void TableManager::push_cell(Cell * cell) {
    ...
    assert(cell->cache_id == -1);
    cell->cache_id = current_cache_id;
    // record cell orders
    this->m_lCachedCells.push_back(cell);
}

py::list TableManager::get_cached_bij() {
    py::list idx2span;
    int total_size = this->m_lCachedCells.size();
    for (int cell_idx = 0; cell_idx < total_size; ++cell_idx) {
        Cell * cell_ptr = this->m_lCachedCells.front();
        this->m_lCachedCells.pop_front();
        idx2span.append(py::make_tuple(cell_ptr->batch_id, cell_ptr->i, cell_ptr->j));
    }
    return idx2span;
}
...
vector<at::Tensor> TableManager::prepare_span_prediction(vector<py::array_t<float>>& split_probabiliy, bool non_overlapping) {
    // list<Cell*> cells_with_span_target;
    std::vector<float> span_weights;
    std::vector<long> span_cache_ids;
    std::vector<long> span_tgt_ids;
    Cell * current = NULL;
    for (int table_i = 0; table_i < this->m_iBatchSize; ++table_i) {
        current = m_pCellTables[table_i]->get(0, m_pCellTables[table_i]->getLen() - 1);
        assert(current != 0);
        current->span_weight = 1.0;
    }
    // Update all cells top-down, thus traversing m_pCellOrders in reverse order.
    assert(split_probabiliy.size() == this->m_iCurrentStep - 1);
    for (int step = this->m_iCurrentStep - 2; step >= 0; --step) {
        auto splits_probs = split_probabiliy[step].unchecked<2>();
        assert(this->m_pCellNums[step] == split_probabiliy[step].shape(0));
        for (int cell_idx = 0; cell_idx < this->m_pCellNums[step]; ++cell_idx) {
            Cell * current_cell = m_pCellOrders[step][cell_idx];
            int table_i = current_cell->batch_id;
            assert(current_cell->i <= current_cell->j);
            assert(current_cell->cache_id != -1);
            if (current_cell->ext_vocab_id > 0) {
                if (!non_overlapping) {
                    // NOTE: current version has no normalization
                    // span_weights.push_back(current_cell->span_weight / (2 * m_pCellTables[table_i]->getLen() - 1));
                    span_weights.push_back(current_cell->span_weight);
                    //span_weights.push_back(current_cell->span_weight * (m_pCellTables[table_i]->getLen() - (current_cell->j - current_cell->i + 1)) * (current_cell->j - current_cell->i + 1) / (m_pCellTables[table_i]->getLen() * m_pCellTables[table_i]->getLen()));
                } else {
                    span_weights.push_back(current_cell->span_weight * (current_cell->j - current_cell->i + 1) / m_pCellTables[table_i]->getLen());
                }
                span_cache_ids.push_back(current_cell->cache_id);
                span_tgt_ids.push_back(current_cell->ext_vocab_id);
                if (non_overlapping) {
                    current_cell->span_weight = 0;
                }
            }
            int splits_num = current_cell->split_size;

            CellTable * tbl = m_pCellTables[current_cell->batch_id];
            for (int split_idx = 0; split_idx < splits_num; ++split_idx) {
                int k = current_cell->splits[split_idx];
                float delta_weight = current_cell->span_weight * splits_probs(cell_idx, split_idx);
                // weight from parent to sub-child

                tbl->get(current_cell->i, k)->span_weight += delta_weight;
                assert(tbl->get(current_cell->i, k)->span_weight <= 1.0001);

                
                tbl->get(k + 1, current_cell->j)->span_weight += delta_weight;
                assert(tbl->get(k + 1, current_cell->j)->span_weight <= 1.0001);
            }
        }
    }

#ifndef NDEBUG
    // test if the weight of each terminal node equals to 1
    if (!non_overlapping) {
        for (int table_i = 0; table_i < this->m_iBatchSize; ++table_i) {
            CellTable * tbl = m_pCellTables[table_i];
            for (int pos = 0; pos < tbl->getLen(); ++pos) {
                assert(tbl->get(pos, pos)->ext_vocab_id != 0);
                assert(tbl->get(pos, pos)->span_weight - 1.0 < 0.0001);
            }
        }
    }
#endif

    return {torch::tensor(span_weights, at::kFloat), torch::tensor(span_cache_ids, at::kLong), torch::tensor(span_tgt_ids, at::kLong)};
}

vector<at::Tensor> TableManager::best_trees(vector<py::array_t<int>>& best_splits, 
                                            vector<py::array_t<int>>& atom_spans, 
                                            const py::array_t<int>& group_ids,
                                            const int max_input_len) {
    assert(best_splits.size() == m_iCurrentStep - 1);
    for (int step = 0; step < this->m_iCurrentStep - 1; ++step) {
        auto splits_matrix = best_splits[step].unchecked<2>();
        int splits_num = best_splits[step].shape(0);
        assert(m_pCellNums[step] == splits_num);
        for (int cell_idx = 0; cell_idx < this->m_pCellNums[step]; ++cell_idx) {
            Cell * current_cell = m_pCellOrders[step][cell_idx];
            assert(current_cell->i <= current_cell->j);
    ...
    for (int batch_i = 0; batch_i < this->m_iBatchSize; ++batch_i) {
        ...
        while (!cell_stack.empty()) {
            ...
            if (current->split_size > 0) {
                ...
                ++split_idx;
                cell_stack.push(tbl->get(current->i, k));
                cell_stack.push(tbl->get(k + 1, current->j));
            }
        }
        sent_offset += sent_len;
    }
    // return span masks, split targets
    return {torch::from_blob(m_pSpanMasks, {group_size, max_input_len - 1, max_input_len - 1}, at::kLong),
            torch::from_blob(m_pSplitTargets, {group_size, max_input_len - 1}, at::kLong)};
}
