...
public:
    ...
    vector<at::Tensor> best_trees(vector<py::array_t<int>>& best_splits, 
                                  vector<py::array_t<int>>& atom_spans, 
                                  const py::array_t<int>& group_ids,
                                  const int max_input_len);
    vector<at::Tensor> prepare_span_prediction(vector<py::array_t<float>>& split_probabiliy, bool non_overlapping=false);
    at::Tensor root_ids();
    at::Tensor prepare_bilm(int total_len, int bos_id, int eos_id);
    const int batch_size() const;
    void on_cell_ready(Cell* cell);
    py::list get_cached_bij();
};
