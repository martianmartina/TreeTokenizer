...
class WordTreeNode:
    ...
    def prune_by(self, condition_fn):
        ...
        for child in list(self.entries.values()):
...
class WordTree:
    ...
    def add(self, ids: List[int], cnt=1):
        ...
        self.total += cnt
        assert len(ids) >= 1
        assert 0 <= ids[0] < len(self.basic_entries), f'ids[0] is {ids[0]} while only {len(self.basic_entries)} entries'
...
def unigram_E_step(pair_path, vocab_model):
    ...
    for pair_i in tqdm(range(len(pair_lens) // 2)):
        indices_len = pair_lens[pair_i * 2]
        ids_len = pair_lens[pair_i * 2 + 1]
        indices = memmap[offset: offset + indices_len] # merge_trajectories
        offset += indices_len
        ids = memmap[offset: offset + ids_len] # input_ids
        offset += ids_len
        if len(ids) == 0:
            continue
