...
def get_spans_from_splits(splits, tokens):
    spans = []
    def backtrack(p, i, j):
        if j == i + 1:
            spans.append((i, i))
            ...
            if j > i + 2:
                ...
                backtrack(p, i, split)
                backtrack(p, split, j)
    backtrack(splits, 0, len(tokens))
    return spans

def get_token_tree_from_splits(splits, tokens):
    """
        :param splits: the split points of span ij
        :param tokens: a sequence of tokens
        :return: stringified token tree
    """
    def backtrack(p, i, j):
        if j == i + 2:
            return '({} {})'.format(tokens[i], tokens[j-1])
        elif j == i + 1:
...
def get_tree_from_merge_trajectory(merge_trajectory: np.array, seq_len, tokens=None, keep_merge_order=False, reverse=False):
    if reverse:
        merge_trajectory[:seq_len-1] = merge_trajectory[:seq_len-1][::-1]
...
def build_trees(seq_lens_np, split_ids_np, cache_ids_np):
    ...
    def _convert_to_cache_id_tree(splits, start, end, cache_indices, leaves_ids):
        split_idx = splits[0]
        if split_idx == -1:
            assert end - start == 1
            return PyNode(None, None, start, end - 1, cache_indices[0])
        else:
            # right_len = end - split_idx - 1
            left_len = split_idx - start + 1
            # postorder traversal
            left_root = _convert_to_cache_id_tree(splits[1: 2 * left_len], start, split_idx + 1, 
                                                  cache_indices[1: 2 * left_len], leaves_ids)
            right_root = _convert_to_cache_id_tree(splits[2 * left_len:], split_idx + 1, end, 
                                                  cache_indices[2 * left_len:], leaves_ids)
            
            root = PyNode(left_root, right_root, start, end - 1, cache_indices[0])
            return root
    ...
    def _on_batch_i(batch_id):
        ...
        return _convert_to_cache_id_tree(splits, 0, ids_len, cache_ids_np[batch_id], leaves_ids)
